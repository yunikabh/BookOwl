...
try{
  ...
  const categoryStats = await Book.aggregate([
    {
      $unwind: "$category", // Unwind the category array to individual category values
    },
    {
      $group:{
        _id:"$category",
    ...
    res.status(200).json(new ApiResponse(200,{totalBooks,totalUsers,categoryStats},"Books by category statistics fetched successfully"))
...
export { addBook, getBooks, getBookById, updateBooks, deleteBooks,newArrivalBooks,dealsOfTheWeek,getBooksStats };
